...
class Server:
    ...
    def accept(self):
        ...
        client_id = data
        if client_id == b'9':
            print_system_notification("am conectat masina")
            self.car_id = "9"
        if client_id == b'0':
            print_system_notification("am conectat owner")
            self.owner_id = "0"
    ...
    def handle_message(self, command):
        message_id = command.split(' ', 1)[1]
        if message_id == "00":
            print_system_notification("lock car")
            self.lock_car()
        elif message_id == "11":
            print_system_notification("unlock car")
            self.unlock_car()
        elif message_id == "22":
            print_system_notification("request car information")
            self.request_info()
        elif message_id.split(' ')[0] == "2":
            print_system_notification("received car info: " + message_id.split(' ', 1)[1])
            #############################
        elif message_id == "30":
            print_system_notification("register renter")
        elif message_id == "31":
            print_system_notification("register owner")
        elif message_id == "32":
            print_system_notification("post car")
        elif message_id == "33":
            print_system_notification("request car")
        elif message_id == "34":
            print_system_notification("start rental")
        elif message_id == "35":
            print_system_notification("end rental")
        else:
            print_system_notification("not understood")

    def request_info(self):
        self.car_locked = False
        self.send_bytes_to_client(self.car_id, "2")

    def unlock_car(self):
        self.car_locked = False
        self.send_bytes_to_client(self.car_id, "1")

    def lock_car(self):
        self.car_locked = True
        self.send_bytes_to_client(self.car_id, "0")
